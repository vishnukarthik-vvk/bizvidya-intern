...
if (!hasRestoredState && !loading && questions.length > 0 ) {
  ...
  if(location.state && Object.keys(previousAnswers).length > 0){
    ...
    if(saved && saved.answers && Object.keys(saved.answers).length > 0){
      ...
      if (userId) {
        try {
          const res = await fetch(`http://localhost:8000/get_progress/${userId}/mcq`);
          if (res.ok) {
...
if (question.category) {
  ...
  if (userId) {
    try {
      const res = await fetch("http://localhost:8000/save_mcq_results", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: parseInt(userId, 10),
  ...
  if (savedToDB) {
    ...
    try {
      await fetch(`http://localhost:8000/clear_progress/${userId}/mcq`, { method: "DELETE" });
    } catch (e) {

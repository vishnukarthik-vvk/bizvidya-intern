...
const saveProgressToDB = async (data) => {
  try {
    ...
    if (!userId) return;
    await fetch("http://localhost:8000/save_progress", {
      method: "POST",
...
console.log("categoryScores:", categoryScores);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`http://localhost:8000/get_progress/${userId}/open_ended`);
        if (!res.ok) return;
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (userId) {
        try {
          const saveRes = await fetch("http://localhost:8000/save_open_ended_results", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              user_id: parseInt(userId, 10),
      ...
      if (savedToDB) {
        ...
        try {
          await fetch(`http://localhost:8000/clear_progress/${userId}/open_ended`, { method: "DELETE" });
        } catch (e) {
